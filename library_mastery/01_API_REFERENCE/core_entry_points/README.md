...
The scripts include a tiny `sys.path` bootstrap so they can be run from a repo checkout without installing packages.

From the repo root:

```bash
python library_mastery/01_API_REFERENCE/core_entry_points/graph/stategraph_basics.py
```
