"""
sqlite_durability_sync_crash_demo.py

Purpose
-------
Demonstrate "durability='sync'" with SQLite checkpointing:
- checkpoints are persisted *synchronously* before the next superstep starts
...
2) Reset DB:
     python codebase_audit/codes/durability/sqlite_durability_sync_crash_demo.py reset

3) Run (this intentionally hard-crashes the process during step 3):
     python codebase_audit/codes/durability/sqlite_durability_sync_crash_demo.py run

4) Inspect what was safely persisted:
     python codebase_audit/codes/durability/sqlite_durability_sync_crash_demo.py inspect

Expected outcome
----------------
Because durability is "sync", you should see that the latest persisted checkpoint
reached count==2 even though the process crashed at the start of step 3.
"""
...
def build_app(checkpointer: SqliteSaver):
    def tick(state: dict) -> dict:
        ...
        # We intentionally crash as soon as step 3 begins (i.e. next_count==3).
        # At this moment, step 2 has already committed its writes, and in "sync"
        # durability mode its checkpoint should already be persisted.
    ...
    builder.add_conditional_edges("tick", loop_or_end)  # returns node name or END
...
def db_path() -> Path:
    ...
    return db_dir / "durability_sync.sqlite"
...
def command_run() -> None:
    ...
    with SqliteSaver.from_conn_string(str(path)) as checkpointer:
        ...
        print("Starting run: durability='sync' (will crash on step 3)")
        app.invoke({"count": 0}, config=config, durability="sync")


def main() -> None:
    if len(sys.argv) != 2 or sys.argv[1] not in {"run", "inspect", "reset"}:
        print("usage: python sqlite_durability_sync_crash_demo.py [run|inspect|reset]")
        raise SystemExit(2)
